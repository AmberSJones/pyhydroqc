...
def get_data(site, sensor, year, path=""):
    ...
    return df_full, df, normal_lbl, srs


df_full, df, normal_lbl, srs = get_data(site, sensor, year, path="/Users/amber/PycharmProjects/LRO-anomaly-detection/LRO_Data/")
df.head()
df.shape
...
fig.add_trace(go.Scatter(x=df.index, y=df.cond, #fig.add_trace adds different types of plots to the same figure.
                    mode='lines',
                    name='cond'))
...
def scale_data(train, test):
    ...
    train[sensor] = scaler.transform(train)
    test[sensor] = scaler.transform(test)
...
def evaluate_model(X_train, X_test):
    ...
    train_mae_loss = pd.DataFrame(np.mean(np.abo(X_train_pred-X_train), axis=1), columns=['Error'])
...
def detect_anomalies(test, test_mae_loss, threshold):
    ...
    Creates data fram of anomalies to explore with more granularity."""
